<?php
    ...
    function setColDirecciones(){
        ...
        $sql00      = "SELECT a.FuCod AS direccion_funcionario, a.FuCodCel1 AS direccion_celular_1_codigo, a.FuCel1 AS direccion_celular_1_numero, a.FuCodCel2 AS direccion_celular_2_codigo, a.FuCel2 AS direccion_celular_2_numero, a.FuPreTelPa AS direccion_telefono_1_codigo, a.FuTelPar AS direccion_telefono_1_numero, a.AiDept AS direccion_departamento, a.ApCiud AS direccion_ciudad, a.AjBarr AS direccion_barrio, a.FuDirPar AS direccion_direccion, a.fumail AS direccion_mail FROM FUNCIONARI a WHERE a.FEst = 'A' AND a.FuCod NOT IN (62)";
        $sql01      = "SELECT FUNPARCOD FROM FUNPAR WHERE FUNPARFUC = (SELECT FUNFICCOD FROM FUNFIC WHERE FUNFICCFU = ?)";
        $sql02      = "INSERT INTO FUNPAR (FUNPAREST, FUNPARTVC, FUNPARTCC, FUNPARTEC, FUNPARTTC, FUNPARFUC, FUNPARCIC, FUNPARBAC, FUNPARCAS, FUNPARCA1, FUNPARCA2, FUNPARCA3, FUNPARUBI, FUNPARTE1, FUNPARCE1, FUNPARCE2, FUNPAREMA, FUNPAROBS, FUNPARAUS, FUNPARAFH, FUNPARAIP) VALUES (?, ?, (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICNOM = ? AND DOMFICVAL = 'PREFIJOCELULAR' AND DOMFICEST = 'H'), (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICNOM = ? AND DOMFICVAL = 'PREFIJOCELULAR' AND DOMFICEST = 'H'), (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICNOM = ? AND DOMFICVAL = 'PREFIJOTELEFONIA' AND DOMFICEST = 'H'), (SELECT FUNFICCOD FROM FUNFIC WHERE FUNFICCFU = ?), (SELECT LOCCIUCOD FROM LOCCIU a INNER JOIN LOCDEP b ON a.LOCCIUDEC = b.LOCDEPCOD WHERE a.LOCCIUEQU = ? AND b.LOCDEPEQU = ?), (SELECT LOCBARCOD FROM LOCBAR a INNER JOIN LOCCIU b ON a.LOCBARCIC = b.LOCCIUCOD INNER JOIN LOCDEP c ON b.LOCCIUDEC = c.LOCDEPCOD WHERE a.LOCBAREQU = ? AND b.LOCCIUEQU = ? AND c.LOCDEPEQU = ?), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
