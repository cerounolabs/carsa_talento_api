<?php
    ...
    function setColAcamedico(){
        ...
        $sql02      = "INSERT INTO FUNACA (FUNACAEST, FUNACATUC, FUNACATCC, FUNACATGC, FUNACATEC, FUNACAFUC, FUNACAOBS, FUNACAAUS, FUNACAAFH, FUNACAAIP) VALUES (?, (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICEQU = ? AND DOMFICVAL = 'UNIVERSIDAD'), (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICEQU = ? AND DOMFICVAL = 'CARRERA'), (SELECT DOMFICCOD FROM DOMFIC WHERE DOMFICNOM = ? AND DOMFICVAL = 'GRADOACADEMICO'), ?, (SELECT FUNFICCOD FROM FUNFIC WHERE FUNFICCFU = ?), ?, ?, ?, ?)";
        ...
        try {
            ...
            while ($rowMSSQL = $stmtMSSQL->fetch()) {
                ...
                if (isset($rowMSSQL['acamedico_grado'])){
