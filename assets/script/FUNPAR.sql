...
CREATE TRIGGER `FUNPARDLT` BEFORE DELETE ON `FUNPAR`
    FOR EACH ROW 
        INSERT INTO `FUNPARA` (`FUNPARAMET`, `FUNPARAUSU`, `FUNPARAFEC`, `FUNPARADIP`, `FUNPARACODOLD`, `FUNPARAESTOLD`, `FUNPARATVCOLD`, `FUNPARATCCOLD`, `FUNPARATECOLD`, `FUNPARATTCOLD`, `FUNPARAFUCOLD`, `FUNPARACICOLD`, `FUNPARABAROLD`, `FUNPARACASOLD`, `FUNPARACA1OLD`, `FUNPARACA2OLD`, `FUNPARACA3OLD`, `FUNPARAUBIOLD`, `FUNPARATE1OLD`, `FUNPARACE1OLD`, `FUNPARACE2OLD`, `FUNPARAEMAOLD`, `FUNPARAOBSOLD`)
        VALUES ('DELETE', OLD.`FUNPARAUS`, NOW(), OLD.`FUNPARAIP`, OLD.`FUNPARCOD`, OLD.`FUNPAREST`, OLD.`FUNPARTVC`, OLD.`FUNPARTCC`, OLD.`FUNPARTEC`, OLD.`FUNPARTTC`, OLD.`FUNPARFUC`, OLD.`FUNPARCIC`, OLD.`FUNPARBAR`, OLD.`FUNPARCAS`, OLD.`FUNPARCA1`, OLD.`FUNPARCA2`, OLD.`FUNPARCA3`, OLD.`FUNPARUBI`, OLD.`FUNPARTE1`, OLD.`FUNPARCE1`, OLD.`FUNPARCE2`, OLD.`FUNPAREMA`, OLD.`FUNPAROBS`);

CREATE TRIGGER `FUNPARINS` AFTER INSERT ON `FUNPAR`
    FOR EACH ROW 
        INSERT INTO `FUNPARA` (`FUNPARAMET`, `FUNPARAUSU`, `FUNPARAFEC`, `FUNPARADIP`, `FUNPARACODNEW`, `FUNPARAESTNEW`, `FUNPARATVCNEW`, `FUNPARATCCNEW`, `FUNPARATECNEW`, `FUNPARATTCNEW`, `FUNPARAFUCNEW`, `FUNPARACICNEW`, `FUNPARABARNEW`, `FUNPARACASNEW`, `FUNPARACA1NEW`, `FUNPARACA2NEW`, `FUNPARACA3NEW`, `FUNPARAUBINEW`, `FUNPARATE1NEW`, `FUNPARACE1NEW`, `FUNPARACE2NEW`, `FUNPARAEMANEW`, `FUNPARAOBSNEW`)
        VALUES ('INSERT', NEW.`FUNPARAUS`, NOW(), NEW.`FUNPARAIP`, NEW.`FUNPARCOD`, NEW.`FUNPAREST`, NEW.`FUNPARTVC`, NEW.`FUNPARTCC`, NEW.`FUNPARTEC`, NEW.`FUNPARTTC`, NEW.`FUNPARFUC`, NEW.`FUNPARCIC`, NEW.`FUNPARBAR`, NEW.`FUNPARCAS`, NEW.`FUNPARCA1`, NEW.`FUNPARCA2`, NEW.`FUNPARCA3`, NEW.`FUNPARUBI`, NEW.`FUNPARTE1`, NEW.`FUNPARCE1`, NEW.`FUNPARCE2`, NEW.`FUNPAREMA`, NEW.`FUNPAROBS`);
...
CREATE TRIGGER `FUNPARUPD` AFTER UPDATE ON `FUNPAR`
    FOR EACH ROW 
        ...
        VALUES ('UPDATE', NEW.`FUNPARAUS`, NOW(), OLD.`FUNPARAIP`, OLD.`FUNPARCOD`, OLD.`FUNPAREST`, OLD.`FUNPARTVC`, OLD.`FUNPARTCC`, OLD.`FUNPARTEC`, OLD.`FUNPARTTC`, OLD.`FUNPARFUC`, OLD.`FUNPARCIC`, OLD.`FUNPARBAR`, OLD.`FUNPARCAS`, OLD.`FUNPARCA1`, OLD.`FUNPARCA2`, OLD.`FUNPARCA3`, OLD.`FUNPARUBI`, OLD.`FUNPARTE1`, OLD.`FUNPARCE1`, OLD.`FUNPARCE2`, OLD.`FUNPAREMA`, OLD.`FUNPAROBS`, NEW.`FUNPARCOD`, NEW.`FUNPAREST`, NEW.`FUNPARTVC`, NEW.`FUNPARTCC`, NEW.`FUNPARTEC`, NEW.`FUNPARTTC`, NEW.`FUNPARFUC`, NEW.`FUNPARCIC`, NEW.`FUNPARBAR`, NEW.`FUNPARCAS`, NEW.`FUNPARCA1`, NEW.`FUNPARCA2`, NEW.`FUNPARCA3`, NEW.`FUNPARUBI`, NEW.`FUNPARTE1`, NEW.`FUNPARCE1`, NEW.`FUNPARCE2`, NEW.`FUNPAREMA`, NEW.`FUNPAROBS`);
