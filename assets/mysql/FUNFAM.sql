CREATE TABLE IF NOT EXISTS `FUNFAM` (
    ...
    `FUNFAMTTC` mediumint(9) NOT NULL COMMENT 'TIPO TELEFONO',
...
CREATE TABLE IF NOT EXISTS `FUNFAMA` (
    ...
    `FUNFAMATTCOLD` mediumint(9) NULL COMMENT 'TIPO TELEFONO OLD',
    ...
    `FUNFAMATTCNEW` mediumint(9) NULL COMMENT 'TIPO TELEFONO NEW',
...
CREATE TRIGGER `FUNFAMDLT` BEFORE DELETE ON `FUNFAM`
    FOR EACH ROW 
        INSERT INTO `FUNFAMA` (`FUNFAMAMET`, `FUNFAMAUSU`, `FUNFAMAFEC`, `FUNFAMADIP`, `FUNFAMACODOLD`, `FUNFAMAESTOLD`, `FUNFAMATPCOLD`, `FUNFAMATCCOLD`, `FUNFAMATTCOLD`, `FUNFAMAFUCOLD`, `FUNFAMANOMOLD`, `FUNFAMAAPEOLD`, `FUNFAMAFHAOLD`, `FUNFAMACICOLD`, `FUNFAMAEMPOLD`, `FUNFAMAOCUOLD`, `FUNFAMACELOLD`, `FUNFAMATELOLD`, `FUNFAMAOBSOLD`) 
        VALUES ('DELETE', OLD.`FUNFAMAUS`, NOW(), OLD.`FUNFAMAIP`, OLD.`FUNFAMCOD`, OLD.`FUNFAMEST`, OLD.`FUNFAMTPC`, OLD.`FUNFAMTCC`, OLD.`FUNFAMTTC`, OLD.`FUNFAMFUC`, OLD.`FUNFAMNOM`, OLD.`FUNFAMAPE`, OLD.`FUNFAMFHA`, OLD.`FUNFAMCIC`, OLD.`FUNFAMEMP`, OLD.`FUNFAMOCU`, OLD.`FUNFAMCEL`, OLD.`FUNFAMTEL`, OLD.`FUNFAMOBS`);

CREATE TRIGGER `FUNFAMINS` AFTER INSERT ON `FUNFAM`
    FOR EACH ROW 
        INSERT INTO `FUNFAMA` (`FUNFAMAMET`, `FUNFAMAUSU`, `FUNFAMAFEC`, `FUNFAMADIP`, `FUNFAMACODNEW`, `FUNFAMAESTNEW`, `FUNFAMATPCNEW`, `FUNFAMATCCNEW`, `FUNFAMATTCNEW`, `FUNFAMAFUCNEW`, `FUNFAMANOMNEW`, `FUNFAMAAPENEW`, `FUNFAMAFHANEW`, `FUNFAMACICNEW`, `FUNFAMAEMPNEW`, `FUNFAMAOCUNEW`, `FUNFAMACELNEW`, `FUNFAMATELNEW`, `FUNFAMAOBSNEW`)
        VALUES ('INSERT', NEW.`FUNFAMAUS`, NOW(), NEW.`FUNFAMAIP`, NEW.`FUNFAMCOD`, NEW.`FUNFAMEST`, NEW.`FUNFAMTPC`, NEW.`FUNFAMTCC`, NEW.`FUNFAMTTC`, NEW.`FUNFAMFUC`, NEW.`FUNFAMNOM`, NEW.`FUNFAMAPE`, NEW.`FUNFAMFHA`, NEW.`FUNFAMCIC`, NEW.`FUNFAMEMP`, NEW.`FUNFAMOCU`, NEW.`FUNFAMCEL`, NEW.`FUNFAMTEL`, NEW.`FUNFAMOBS`);
...
ALTER TABLE `FUNFAM` ADD KEY `FUNFAMTTC` (`FUNFAMTTC`);
ALTER TABLE `FUNFAM` ADD CONSTRAINT `FK_FUNFAM_FUNFAMTTC` FOREIGN KEY (`FUNFAMTTC`) REFERENCES `DOMFIC` (`DOMFICCOD`) ON DELETE RESTRICT ON UPDATE RESTRICT;
