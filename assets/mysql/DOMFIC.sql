CREATE TABLE IF NOT EXISTS `DOMFIC` (
...
CREATE TABLE IF NOT EXISTS `DOMFICA` (
...
CREATE TRIGGER `DOMFICDLT` BEFORE DELETE ON `DOMFIC`
  FOR EACH ROW 
    ...
    VALUES ('DELETE', OLD.`DOMFICAUS`, NOW(), OLD.`DOMFICAIP`, OLD.`DOMFICCOD`, OLD.`DOMFICEST`, OLD.`DOMFICNOM`, OLD.`DOMFICEQU`, OLD.`DOMFICVAL`, OLD.`DOMFICOBS`);
...
CREATE TRIGGER `DOMFICINS` AFTER INSERT ON `DOMFIC`
  FOR EACH ROW 
    ...
    VALUES ('INSERT', NEW.`DOMFICAUS`, NOW(), NEW.`DOMFICAIP`, NEW.`DOMFICCOD`, NEW.`DOMFICEST`, NEW.`DOMFICNOM`, NEW.`DOMFICEQU`, NEW.`DOMFICVAL`, NEW.`DOMFICOBS`);
...
CREATE TRIGGER `DOMFICUPD` AFTER UPDATE ON `DOMFIC`
  FOR EACH ROW 
    ...
    VALUES ('UPDATE', NEW.`DOMFICAUS`, NOW(), NEW.`DOMFICAIP`, OLD.`DOMFICCOD`, NEW.`DOMFICCOD`, OLD.`DOMFICEST`, NEW.`DOMFICEST`, OLD.`DOMFICNOM`, NEW.`DOMFICNOM`, OLD.`DOMFICEQU`, NEW.`DOMFICEQU`, OLD.`DOMFICVAL`, NEW.`DOMFICVAL`, OLD.`DOMFICOBS`, NEW.`DOMFICOBS`);
